...
# General
  ...
  OutFile "autolainaushallintaInstaller.exe"
  Unicode True
...
# Interface Settings
  ...
  # Add different icon from NSIS folder
...
Section "Autolainaussovellus hallinta" SecProgram

  SetOutPath "$INSTDIR"

  # READ FILES RECURSIVELY FROM DISTRIBUTION FOLDER CREATED AND PYINSTALLER
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
    ...
    CreateShortcut "$SMPROGRAMS\$StartMenuFolder\Autolainaushallinta.lnk" "$INSTDIR\autonlainaushallinta.exe"
...
; # Section for optional file or directories 
; Section "NameOfFile" SecAdditions
; SetOutPath "$INSTDIR\AdditionsFolderName"
  
; # Files to put into Additional files subdirectory Dictionaries
; File "*.something"
; File "*.otherType"
; SectionEnd
...
# Descriptions
  ...
  LangString DESC_SecProgram ${LANG_ENGLISH} "A program to administrative vehicle and users"
  ; LangString DESC_SecAdditions ${LANG_FINNISH} "Additional content explained in finnish"
  ; LangString DESC_SecAdditions ${LANG_ENGLISH} "Additional context explained in english"
